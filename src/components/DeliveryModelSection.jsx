...
const ChallengesSection = () => {
  ...
  const approaches = [
    "Outcome-driven sprints with fixed scope and explicit acceptance replace time- and headcount-based delivery.",
    "Ownership-first delivery with complete IP transfer, bounded support, and frictionless exit by design.",
    "We embed domain experts into your workflows to minimize handoffs, accelerate delivery, and move AI into production faster—designed to amplify your teams, not substitute them.",
    "Delivery capacity scales by adjusting sprints, without disrupting teams—working alongside you as priorities evolve."
  ];
    ...
    <section ref={sectionRef} className="py-24 bg-gray-900 text-white relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative grid lg:grid-cols-2 gap-16 items-start">
          {/* Vertical line in the middle - positioned absolutely from this container */}
          <div className="hidden lg:block absolute left-1/2 top-0 bottom-0 w-[2px] bg-brand-gray-800/50 transform -translate-x-1/2 z-0"></div>
          <motion.div
            className="hidden lg:block absolute left-1/2 top-0 w-[2px] transform -translate-x-1/2 z-0"
            style={{
          ...
          >
            ...
            <p className="text-xl text-gray-400 leading-relaxed">
              {activeTab === 'challenges'
                ...
                : "We embed with your team to build production-grade AI systems through rapid iteration, avoiding common pitfalls while ensuring full knowledge transfer and ownership."}
            </p>
          </motion.div>
          ...
          <div className="hidden lg:block absolute left-1/2 top-0 bottom-0 pointer-events-none" style={{ width: '0px', zIndex: 20, transform: 'translateX(calc(-2rem + 22px))' }}>
            {items.map((item, index) => (
              <motion.div
                key={`circle-${index}`}
                ...
                style={{
                  width: '20px',
                  height: '20px',
                  top: `${index * 112 + 4}px`, // Position based on spacing (16 * 7 = 112px between items)
                  left: '50%',
